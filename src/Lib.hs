    ...
    leapYear :: Word -> Bool
    leapYear year = (mod year 4 == 0 &&  not (mod year 100 == 0)) || (mod year 100 == 0 && mod year 400 == 0)
    ...
    distance2D (x1, y1) (x2, y2) = sqrt ((x1 - x2) ^ 2 + (y1 - y2) ^ 2)
    ...
    natToBinstring x = natToBinstring(div x 2) ++ show (mod x 2)
